...
static xt_core_bool_t ensure_client_is_connected(xt_net_client_system_t *client);
...
xt_core_bool_t connect_to_server(xt_net_client_system_t *client)
{
  ...
  for (port = client->server_min_port;
    ...
    if (client->socket >= 0) {
      ...
      xt_core_log_enter(client->log, "hnet",
      ...
      pthread_mutex_lock(&client->messaging_mutex);
      {
        ...
        if (client->post) {
          if (xt_net_exchange_register_post
            ...
            xt_core_log_trace(client->log, "hnet",
          ...
          xt_core_log_trace(client->log, "hnet", "x_net_post_create");
          xt_net_client_socket_destroy(client->socket);
          client->server_socket_closed = xt_core_bool_true;
        }
      }
...
xt_core_bool_t ensure_client_is_connected(xt_net_client_system_t *client)
{
  ...
  if (client->server_socket_closed) {
    ...
    if (connected) {
      xt_core_log_enter(client->log, "hnet", "reconnected to server");
    } else {
      xt_core_log_enter(client->log, "hnet", "could not reconnect to server");
    }
  }
...
xt_core_bool_t handle_disconnect(xt_net_client_system_t *client)
{
  ...
  xt_core_log_enter(client->log, "hnet", "lost connection to server");
  ...
  if (xt_net_exchange_unregister_post(client->exchange, client->socket)) {
    ...
    success = xt_core_bool_false;
    xt_core_log_trace(client->log, "hnet", "x_net_exchange_unregister_post");
  }
...
xt_net_client_system_t *xt_net_client_system_create(const char *server_ip_address,
    unsigned short server_min_port, unsigned short server_max_port,
    xt_net_engine_get_name_f get_engine_name, void *custom_client_context,
    xt_core_log_t *log)
{
  ...
  if (client) {
    ...
    success = xt_core_bool_false;
    xt_core_log_trace(client->log, "hnet", "malloc");
  }
  ...
  if (success) {
    if (0 == pthread_mutex_init(&client->messaging_mutex, NULL)) {
      ...
      success = xt_core_bool_false;
      xt_core_log_trace(client->log, "hnet", "pthread_mutex_init");
    }
  }
  ...
  if (success) {
    ...
    if (!connected) {
      success = xt_core_bool_false;
      xt_core_log_trace(client->log, "hnet", "connect_to_server");
    }
  }
...
char *xt_net_client_system_get_server_ip_address(xt_net_client_system_t *client)
{
  return client->server_ip_address;
}

unsigned short xt_net_client_system_get_server_port(xt_net_client_system_t *client)
{
...
xt_core_bool_t xt_net_client_system_is_connected_to_server(xt_net_client_system_t *client)
{
...
void xt_net_client_system_process_messages(xt_net_client_system_t *client)
{
  ...
  if (ensure_client_is_connected(client)) {
    ...
    xt_core_log_trace(client->log, "hnet", "ensure_client_is_connected");
  }
}

xt_core_bool_t xt_net_client_system_register_engine(xt_net_client_system_t *client,
...
void xt_net_client_system_register_message_handler(xt_net_client_system_t *client,
...
xt_core_bool_t xt_net_client_system_send_message(xt_net_client_system_t *client,
    xt_core_message_t *message)
{
  xt_core_bool_t success;
      ...
      if (xt_net_post_system_send_message(client->post, message)) {
        ...
        if (xt_net_post_system_is_socket_closed(client->post)) {
          ...
          if (!handle_disconnect(client)) {
            xt_core_log_trace(client->log, "hnet", "handle_disconnect");
          }
        }
      } else {
        success = xt_core_bool_false;
        xt_core_log_trace(client->log, "hnet", "x_net_post_send_message");
      }
    }
    pthread_mutex_unlock(&client->messaging_mutex);
...
xt_case_list_t *xt_net_client_system_take_unsent_messages(xt_net_client_system_t *client)
{
  assert(client);
